...
public class SchooltestCourseDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestCourseDAO instance = null;

	public static SchooltestCourseDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestCourseDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestCourseDAO getNewInstance(String dbName) {
		return new SchooltestCourseDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestCourseDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestCourseDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.course", dbName);
	...
	public int insertReturnId(SchooltestCourse obj) {
		String sql = String.format("INSERT INTO %s.course(id,courseName,teacherName,majorName,semesterId,semesterName,url) VALUES ", dbName);
	...
	public void insert(SchooltestCourse obj) {
		String sql = String.format("INSERT INTO %s.course(id,courseName,teacherName,majorName,semesterId,semesterName,url) VALUES (?,?,?,?,?,?,?)", dbName);
	 ...
	 */
	@Deprecated
	public void insertBatch(List<SchooltestCourse> objList) {
		String sql = String.format("INSERT INTO %s.course(id,courseName,teacherName,majorName,semesterId,semesterName,url) VALUES (?,?,?,?,?,?,?)", dbName);
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestCourse obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultiple(List<SchooltestCourse> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.course(id,courseName,teacherName,majorName,semesterId,semesterName,url) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<SchooltestCourse> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.course(id,courseName,teacherName,majorName,semesterId,semesterName,url) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestCourse obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestCourse> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestCourse] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestCourse> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestCourse> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestCourse] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	 ...
	 */
	public void update(SchooltestCourse obj) {
		String sql = String.format("UPDATE %s.course SET id = ?,courseName = ?,teacherName = ?,majorName = ?,semesterId = ?,semesterName = ?,url = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	 ...
	 */
	public void updateBatch(List<SchooltestCourse> objList) {
		String sql = String.format("UPDATE %s.course SET id = ?,courseName = ?,teacherName = ?,majorName = ?,semesterId = ?,semesterName = ?,url = ? WHERE id = ?", dbName);
		...
		try {
			...
			for (SchooltestCourse obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestCourse] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestCourse> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.course SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE course SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.course SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE course SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.course WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestCourse] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.course WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestCourse] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.course", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestCourse selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE id = ?", dbName);
		SchooltestCourse result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestCourse> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.course WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
	...
	public List<SchooltestCourse> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestCourse selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestCourse result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestCourse> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE %s = ?", dbName, field);
		List<SchooltestCourse> result = new ArrayList<SchooltestCourse>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestCourse> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE %s LIKE ?", dbName, field);
		List<SchooltestCourse> result = new ArrayList<SchooltestCourse>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestCourse> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.course WHERE %s IN (", dbName, field));
	...
	public List<SchooltestCourse> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestCourse selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestCourse> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.course WHERE %s = ?", dbName, field);
	...
	public List<SchooltestCourse> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestCourse>();
	...
	public List<SchooltestCourse> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestCourse> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestCourse> result = new ArrayList<SchooltestCourse>();
		...
		String sql = String.format("SELECT %s FROM %s.course WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestCourse obj = SqlConstructUtil._constructResult(fieldList, SchooltestCourse.class, rs);
	...
	public List<SchooltestCourse> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestCourse> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestCourse> walkAll(int walkstep) {
	...
	public List<SchooltestCourse> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestCourse> result = new ArrayList<SchooltestCourse>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestCourse> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestCourse] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestCourse> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestCourse> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestCourse> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestCourse> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestCourse> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestCourse] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestCourse.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, SchooltestCourse obj, int indexCount) throws SQLException {
	...
	public SchooltestCourse _constructResult(ResultSet rs) throws SQLException {
		SchooltestCourse obj = new SchooltestCourse();
		obj.setId(rs.getInt("id"));
