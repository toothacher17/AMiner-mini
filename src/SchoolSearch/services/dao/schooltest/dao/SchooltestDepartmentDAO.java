...
public class SchooltestDepartmentDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestDepartmentDAO instance = null;

	public static SchooltestDepartmentDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestDepartmentDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestDepartmentDAO getNewInstance(String dbName) {
		return new SchooltestDepartmentDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestDepartmentDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestDepartmentDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.department", dbName);
	...
	public int insertReturnId(SchooltestDepartment obj) {
		String sql = String.format("INSERT INTO %s.department(id,name) VALUES ", dbName);
	...
	public void insert(SchooltestDepartment obj) {
		String sql = String.format("INSERT INTO %s.department(id,name) VALUES (?,?)", dbName);
	 ...
	 */
	@Deprecated
	public void insertBatch(List<SchooltestDepartment> objList) {
		String sql = String.format("INSERT INTO %s.department(id,name) VALUES (?,?)", dbName);
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestDepartment obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultiple(List<SchooltestDepartment> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.department(id,name) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<SchooltestDepartment> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.department(id,name) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestDepartment obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestDepartment> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestDepartment] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestDepartment> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestDepartment> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestDepartment] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	 ...
	 */
	public void update(SchooltestDepartment obj) {
		String sql = String.format("UPDATE %s.department SET id = ?,name = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	 ...
	 */
	public void updateBatch(List<SchooltestDepartment> objList) {
		String sql = String.format("UPDATE %s.department SET id = ?,name = ? WHERE id = ?", dbName);
		...
		try {
			...
			for (SchooltestDepartment obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestDepartment] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestDepartment> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.department SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE department SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.department SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE department SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.department WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestDepartment] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.department WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestDepartment] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.department", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestDepartment selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE id = ?", dbName);
		SchooltestDepartment result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestDepartment> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		} else if (idList.size() == 0) {
			return new ArrayList<SchooltestDepartment>();
		}

		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.department WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
		...
		List<SchooltestDepartment> result = new ArrayList<SchooltestDepartment>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestDepartment> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestDepartment selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestDepartment result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestDepartment> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE %s = ?", dbName, field);
		List<SchooltestDepartment> result = new ArrayList<SchooltestDepartment>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestDepartment> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE %s LIKE ?", dbName, field);
		List<SchooltestDepartment> result = new ArrayList<SchooltestDepartment>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestDepartment> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.department WHERE %s IN (", dbName, field));
	...
	public List<SchooltestDepartment> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestDepartment selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestDepartment> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.department WHERE %s = ?", dbName, field);
	...
	public List<SchooltestDepartment> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestDepartment>();
	...
	public List<SchooltestDepartment> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestDepartment> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestDepartment> result = new ArrayList<SchooltestDepartment>();
		...
		String sql = String.format("SELECT %s FROM %s.department WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestDepartment obj = SqlConstructUtil._constructResult(fieldList, SchooltestDepartment.class, rs);
	...
	public List<SchooltestDepartment> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestDepartment> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestDepartment> walkAll(int walkstep) {
	...
	public List<SchooltestDepartment> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestDepartment> result = new ArrayList<SchooltestDepartment>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestDepartment> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestDepartment] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestDepartment> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestDepartment> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestDepartment> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestDepartment> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestDepartment> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestDepartment] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestDepartment.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, SchooltestDepartment obj, int indexCount) throws SQLException {
	...
	public SchooltestDepartment _constructResult(ResultSet rs) throws SQLException {
		SchooltestDepartment obj = new SchooltestDepartment();
		obj.setId(rs.getInt("id"));
