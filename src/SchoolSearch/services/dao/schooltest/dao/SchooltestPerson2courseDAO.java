...
public class SchooltestPerson2courseDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestPerson2courseDAO instance = null;

	public static SchooltestPerson2courseDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestPerson2courseDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestPerson2courseDAO getNewInstance(String dbName) {
		return new SchooltestPerson2courseDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestPerson2courseDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestPerson2courseDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.person2course", dbName);
	...
	public void insertBatch(List<SchooltestPerson2course> objList) {
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestPerson2course obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultipleBatch(List<SchooltestPerson2course> objList, int multipleSize) {
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestPerson2course obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestPerson2course> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestPerson2course] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestPerson2course> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestPerson2course> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestPerson2course] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	...
	public void updateBatch(List<SchooltestPerson2course> objList) {
		...
		try {
			...
			for (SchooltestPerson2course obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestPerson2course] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestPerson2course> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.person2course SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE person2course SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.person2course SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE person2course SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.person2course WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestPerson2course] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.person2course WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestPerson2course] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.person2course", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestPerson2course selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE id = ?", dbName);
		SchooltestPerson2course result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2course> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		} else if (idList.size() == 0) {
			return new ArrayList<SchooltestPerson2course>();
		}

		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.person2course WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
		...
		List<SchooltestPerson2course> result = new ArrayList<SchooltestPerson2course>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2course> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestPerson2course selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestPerson2course result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2course> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE %s = ?", dbName, field);
		List<SchooltestPerson2course> result = new ArrayList<SchooltestPerson2course>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2course> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE %s LIKE ?", dbName, field);
		List<SchooltestPerson2course> result = new ArrayList<SchooltestPerson2course>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2course> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.person2course WHERE %s IN (", dbName, field));
	...
	public List<SchooltestPerson2course> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestPerson2course selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestPerson2course> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2course WHERE %s = ?", dbName, field);
	...
	public List<SchooltestPerson2course> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestPerson2course>();
	...
	public List<SchooltestPerson2course> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestPerson2course> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestPerson2course> result = new ArrayList<SchooltestPerson2course>();
		...
		String sql = String.format("SELECT %s FROM %s.person2course WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestPerson2course obj = SqlConstructUtil._constructResult(fieldList, SchooltestPerson2course.class, rs);
	...
	public List<SchooltestPerson2course> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestPerson2course> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestPerson2course> walkAll(int walkstep) {
	...
	public List<SchooltestPerson2course> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestPerson2course> result = new ArrayList<SchooltestPerson2course>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson2course> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestPerson2course] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestPerson2course> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestPerson2course> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestPerson2course> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestPerson2course> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson2course> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestPerson2course] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestPerson2course.class.getDeclaredField(fieldName));
			}
	...
	public SchooltestPerson2course _constructResult(ResultSet rs) throws SQLException {
