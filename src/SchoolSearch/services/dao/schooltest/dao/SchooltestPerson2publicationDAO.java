...
public class SchooltestPerson2publicationDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestPerson2publicationDAO instance = null;

	public static SchooltestPerson2publicationDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestPerson2publicationDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestPerson2publicationDAO getNewInstance(String dbName) {
		return new SchooltestPerson2publicationDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestPerson2publicationDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestPerson2publicationDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.person2publication", dbName);
	...
	public int insertReturnId(SchooltestPerson2publication obj) {
		String sql = String.format("INSERT INTO %s.person2publication(id,publication_id,person_id,position,score,label,confirmed) VALUES ", dbName);
	...
	public void insertBatch(List<SchooltestPerson2publication> objList) {
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestPerson2publication obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultiple(List<SchooltestPerson2publication> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.person2publication(id,publication_id,person_id,position,score,label,confirmed) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<SchooltestPerson2publication> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.person2publication(id,publication_id,person_id,position,score,label,confirmed) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestPerson2publication obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestPerson2publication> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestPerson2publication] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestPerson2publication> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestPerson2publication> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestPerson2publication] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	...
	public void update(SchooltestPerson2publication obj) {
		String sql = String.format("UPDATE %s.person2publication SET id = ?,publication_id = ?,person_id = ?,position = ?,score = ?,label = ?,confirmed = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	...
	public void updateBatch(List<SchooltestPerson2publication> objList) {
		...
		try {
			...
			for (SchooltestPerson2publication obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestPerson2publication] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestPerson2publication> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.person2publication SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE person2publication SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.person2publication SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE person2publication SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.person2publication WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestPerson2publication] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.person2publication WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestPerson2publication] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.person2publication", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestPerson2publication selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE id = ?", dbName);
		SchooltestPerson2publication result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2publication> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		} else if (idList.size() == 0) {
			return new ArrayList<SchooltestPerson2publication>();
		}

		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.person2publication WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
	...
	public List<SchooltestPerson2publication> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestPerson2publication selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestPerson2publication result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2publication> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE %s = ?", dbName, field);
		List<SchooltestPerson2publication> result = new ArrayList<SchooltestPerson2publication>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2publication> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE %s LIKE ?", dbName, field);
		List<SchooltestPerson2publication> result = new ArrayList<SchooltestPerson2publication>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson2publication> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.person2publication WHERE %s IN (", dbName, field));
	...
	public List<SchooltestPerson2publication> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestPerson2publication selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestPerson2publication> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person2publication WHERE %s = ?", dbName, field);
	...
	public List<SchooltestPerson2publication> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestPerson2publication>();
	...
	public List<SchooltestPerson2publication> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestPerson2publication> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestPerson2publication> result = new ArrayList<SchooltestPerson2publication>();
		...
		String sql = String.format("SELECT %s FROM %s.person2publication WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestPerson2publication obj = SqlConstructUtil._constructResult(fieldList, SchooltestPerson2publication.class, rs);
	...
	public List<SchooltestPerson2publication> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestPerson2publication> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestPerson2publication> walkAll(int walkstep) {
	...
	public List<SchooltestPerson2publication> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestPerson2publication> result = new ArrayList<SchooltestPerson2publication>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson2publication> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestPerson2publication] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestPerson2publication> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestPerson2publication> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestPerson2publication> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestPerson2publication> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson2publication> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestPerson2publication] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestPerson2publication.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, SchooltestPerson2publication obj, int indexCount) throws SQLException {
		...
		SqlConstructUtil.__safeSetString(ps, ++indexCount, obj.getScore());
	...
	public SchooltestPerson2publication _constructResult(ResultSet rs) throws SQLException {
		SchooltestPerson2publication obj = new SchooltestPerson2publication();
		obj.setId(rs.getInt("id"));
