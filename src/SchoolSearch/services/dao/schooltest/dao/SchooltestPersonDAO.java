...
public class SchooltestPersonDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestPersonDAO instance = null;

	public static SchooltestPersonDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestPersonDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestPersonDAO getNewInstance(String dbName) {
		return new SchooltestPersonDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestPersonDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestPersonDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.person", dbName);
	...
	public int insertReturnId(SchooltestPerson obj) {
		String sql = String.format("INSERT INTO %s.person(id,name,name_alias) VALUES ", dbName);
	...
	public void insert(SchooltestPerson obj) {
		String sql = String.format("INSERT INTO %s.person(id,name,name_alias) VALUES (?,?,?)", dbName);
	 ...
	 */
	@Deprecated
	public void insertBatch(List<SchooltestPerson> objList) {
		String sql = String.format("INSERT INTO %s.person(id,name,name_alias) VALUES (?,?,?)", dbName);
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestPerson obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultiple(List<SchooltestPerson> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.person(id,name,name_alias) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<SchooltestPerson> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.person(id,name,name_alias) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestPerson obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestPerson> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestPerson] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestPerson> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestPerson> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestPerson] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	 ...
	 */
	public void update(SchooltestPerson obj) {
		String sql = String.format("UPDATE %s.person SET id = ?,name = ?,name_alias = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	 ...
	 */
	public void updateBatch(List<SchooltestPerson> objList) {
		String sql = String.format("UPDATE %s.person SET id = ?,name = ?,name_alias = ? WHERE id = ?", dbName);
		...
		try {
			...
			for (SchooltestPerson obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestPerson] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestPerson> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.person SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE person SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.person SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE person SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.person WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestPerson] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.person WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestPerson] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.person", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestPerson selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE id = ?", dbName);
		SchooltestPerson result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.person WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
	...
	public List<SchooltestPerson> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestPerson selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestPerson result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE %s = ?", dbName, field);
		List<SchooltestPerson> result = new ArrayList<SchooltestPerson>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE %s LIKE ?", dbName, field);
		List<SchooltestPerson> result = new ArrayList<SchooltestPerson>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestPerson> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.person WHERE %s IN (", dbName, field));
	...
	public List<SchooltestPerson> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestPerson selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestPerson> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.person WHERE %s = ?", dbName, field);
	...
	public List<SchooltestPerson> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestPerson>();
	...
	public List<SchooltestPerson> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestPerson> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestPerson> result = new ArrayList<SchooltestPerson>();
		...
		String sql = String.format("SELECT %s FROM %s.person WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestPerson obj = SqlConstructUtil._constructResult(fieldList, SchooltestPerson.class, rs);
	...
	public List<SchooltestPerson> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestPerson> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestPerson> walkAll(int walkstep) {
	...
	public List<SchooltestPerson> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestPerson> result = new ArrayList<SchooltestPerson>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestPerson] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestPerson> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestPerson> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestPerson> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestPerson> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestPerson> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestPerson] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestPerson.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, SchooltestPerson obj, int indexCount) throws SQLException {
		SqlConstructUtil.__safeSetInt(ps, ++indexCount, obj.getId());
		SqlConstructUtil.__safeSetString(ps, ++indexCount, obj.getName());
		SqlConstructUtil.__safeSetString(ps, ++indexCount, obj.getNameAlias());

		return indexCount;
	}

	public SchooltestPerson _constructResult(ResultSet rs) throws SQLException {
		SchooltestPerson obj = new SchooltestPerson();
		obj.setId(rs.getInt("id"));
		obj.setName(rs.getString("name"));
		obj.setNameAlias(rs.getString("name_alias"));
