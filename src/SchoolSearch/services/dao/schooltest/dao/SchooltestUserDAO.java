...
import SchoolSearch.services.dao.schooltest.model.SchooltestUser;
...
public class SchooltestUserDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static SchooltestUserDAO instance = null;

	public static SchooltestUserDAO getInstance() {
		if (null == instance) {
			instance = new SchooltestUserDAO("schooltest");
		}
		return instance;
	}
	
	public static SchooltestUserDAO getNewInstance(String dbName) {
		return new SchooltestUserDAO(dbName);
	}
	
	private final String dbName;

	public SchooltestUserDAO() {
		this.dbName = "schooltest";
	}
	
	private SchooltestUserDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.user", dbName);
	...
	public int insertReturnId(SchooltestUser obj) {
		String sql = String.format("INSERT INTO %s.user(id,username,phone,mailAddress,userType,password) VALUES ", dbName);
	...
	public void insert(SchooltestUser obj) {
		String sql = String.format("INSERT INTO %s.user(id,username,phone,mailAddress,userType,password) VALUES (?,?,?,?,?,?)", dbName);
	 ...
	 */
	@Deprecated
	public void insertBatch(List<SchooltestUser> objList) {
		String sql = String.format("INSERT INTO %s.user(id,username,phone,mailAddress,userType,password) VALUES (?,?,?,?,?,?)", dbName);
		...
		try {
			...
			ps = conn.prepareStatement(sql);
			for (SchooltestUser obj : objList) {
				_constructPS(ps, obj, 0);
				ps.addBatch();
			}
	...
	public void insertMultiple(List<SchooltestUser> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.user(id,username,phone,mailAddress,userType,password) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<SchooltestUser> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.user(id,username,phone,mailAddress,userType,password) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (SchooltestUser obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<SchooltestUser> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting SchooltestUser] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<SchooltestUser> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<SchooltestUser> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			System.out.println(String.format("[inserting SchooltestUser] %d / %d, cost %d ms, total estimation %d ms", startIndex, objList.size(), //
	 ...
	 */
	public void update(SchooltestUser obj) {
		String sql = String.format("UPDATE %s.user SET id = ?,username = ?,phone = ?,mailAddress = ?,userType = ?,password = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	 ...
	 */
	public void updateBatch(List<SchooltestUser> objList) {
		String sql = String.format("UPDATE %s.user SET id = ?,username = ?,phone = ?,mailAddress = ?,userType = ?,password = ? WHERE id = ?", dbName);
		...
		try {
			...
			for (SchooltestUser obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating SchooltestUser] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatch(List<SchooltestUser> objList, List<Field> updateFields) {
		StringBuilder sqlBuilder = new StringBuilder();
		sqlBuilder.append(String.format("UPDATE %s.user SET ", dbName));
	...
	/**
	 * 	Batch(<b><i>i</i></b> ):
	 *		UPDATE user SET <b>setField</b> = <b>setToValueList[<i>i</i> ]</b> WHERE <b>whereField</b> IN 
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, List<List<Integer>> whereInValueList, //
		...
		try {
			...
			for (int i = 0; i < whereInValueList.size(); i++) {
				...
				StringBuilder sqlBuilder = new StringBuilder( //
						String.format("UPDATE %s.user SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	/**
	 * UPDATE user SET <b>setField</b> = <b>setToValue</b> WHERE <b>whereField</b> IN (<b>whereInValue[0 : length]</b>)
	 */
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.user WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting SchooltestUser] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			SqlConstructUtil._executeBatch(ps);
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.user WHERE id IN (");
	...
	public void deleteMultipleBatchByIdWithLimitedWindow(List<Integer> idList, int batchWindowSize, int batchMount) {
		...
		while (startIndex < idList.size()) {
			...
			startIndex += batchMount * batchWindowSize;
			System.out.println(String.format("[deleting SchooltestUser] %d / %d, cost %d ms, total estimation %d ms", startIndex, idList.size(), //
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.user", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public SchooltestUser selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE id = ?", dbName);
		SchooltestUser result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestUser> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		} else if (idList.size() == 0) {
			return new ArrayList<SchooltestUser>();
		}

		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.user WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
		...
		List<SchooltestUser> result = new ArrayList<SchooltestUser>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestUser> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public SchooltestUser selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE %s = ? LIMIT 1", dbName, field);
		SchooltestUser result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestUser> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE %s = ?", dbName, field);
		List<SchooltestUser> result = new ArrayList<SchooltestUser>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestUser> selectLikeStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE %s LIKE ?", dbName, field);
		List<SchooltestUser> result = new ArrayList<SchooltestUser>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<SchooltestUser> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.user WHERE %s IN (", dbName, field));
	...
	public List<SchooltestUser> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public SchooltestUser selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<SchooltestUser> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.user WHERE %s = ?", dbName, field);
	...
	public List<SchooltestUser> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<SchooltestUser>();
	...
	public List<SchooltestUser> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<SchooltestUser> walk(List<Field> fieldList, int start, int limit) {
		List<SchooltestUser> result = new ArrayList<SchooltestUser>();
		...
		String sql = String.format("SELECT %s FROM %s.user WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					SchooltestUser obj = SqlConstructUtil._constructResult(fieldList, SchooltestUser.class, rs);
	...
	public List<SchooltestUser> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<SchooltestUser> walkAll() {
		return walkAll(null, -1);
	}

	public List<SchooltestUser> walkAll(int walkstep) {
	...
	public List<SchooltestUser> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<SchooltestUser> result = new ArrayList<SchooltestUser>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestUser> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading SchooltestUser] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<SchooltestUser> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<SchooltestUser> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<SchooltestUser> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<SchooltestUser> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<SchooltestUser> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching SchooltestUser] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(SchooltestUser.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, SchooltestUser obj, int indexCount) throws SQLException {
		...
		SqlConstructUtil.__safeSetString(ps, ++indexCount, obj.getMailAddress());
	...
	public SchooltestUser _constructResult(ResultSet rs) throws SQLException {
		SchooltestUser obj = new SchooltestUser();
		obj.setId(rs.getInt("id"));
