...
public class RerankServiceImpl3 implements RerankService {
	...
	public List<Integer> personRerank(List<ScoreDoc> searchResult, IndexSearcher searcher) {
		...
		List<RerankObject> rerankObjectList = new ArrayList<RerankServiceImpl3.RerankObject>();
		Map<Integer, RerankObject> rerankMap = new HashMap<Integer, RerankServiceImpl3.RerankObject>();
		...
		for (ScoreDoc sd : searchResult) {
			...
			Double rebundanceScore = Double.parseDouble(doc.get("personinfo.score"));
			RerankObject rerankObject = new RerankObject(pid, sd.score, rebundanceScore);
	...
	private static class RerankObject {
		...
		public RerankObject(Integer id, Float similarityScore, Double rebundanceScore) {
			super();
			this.id = id;
		...
		public static Comparator<RerankObject> defailt_comparator = new Comparator<RerankServiceImpl3.RerankObject>() {
			@Override
